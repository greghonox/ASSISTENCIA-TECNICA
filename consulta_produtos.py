...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_produto.ui'
#
...
class Consulta_produtos(object):
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA PRODUTOS(EQUIPAMENTOS)- ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA PRODUTOS(EQUIPAMENTOS)"))
        ...
        self.filtro.setItemText(1, _translate("Form", "PRODUTO"))
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "PRODUTOS", self.pegar_dados_tabela())
        Form.exec()
    ...
    unidades = ["PEÇA(PC)", "CAIXA(CX)", "PACOTE(PCT)", "UNIDADE(UN)"]
    def carregar_tabela(self, opcao=0):
        SQL = sql.SQL()
        busca = self.busca.text()
        lim = self.limite.value()

        opcao = SQL.pegar_produtos(lim, self.opcao_filtro) if(opcao == 0) else \
        SQL.pegar_produtos_nome(busca, lim, self.opcao_filtro) if(opcao == 1) else SQL.pegar_produtos_codigo(busca, lim)\
        if(opcao == 2) else SQL.pegar_produtos_descricao(busca, lim, self.opcao_filtro)
        ...
        for col, dados in enumerate(opcao):
            ...
            for lin, usu in enumerate(dados):
                ...
                usu = self.unidades[usu] if(lin == 3) else usu
