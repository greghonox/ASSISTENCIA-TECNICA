...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_usuario.ui'
#
...
class Consulta_usuarios(object):
    def setupUi(self, Form):
        Form.setObjectName("Form")
        Form.resize(943, 526)
        Form.setMinimumSize(QtCore.QSize(889, 469))
        Form.setMaximumSize(QtCore.QSize(943, 526))
        icon = QtGui.QIcon()
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA USUÁRIO - ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA USUÁRIO"))
        ...
        self.filtro.setItemText(1, _translate("Form", "USUARIO"))
    ...
    def carregar_janela(self, Form):
        ...
        self.tabela.doubleClicked.connect(self.alterar_usuario)
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "USUARIOS", self.pegar_dados_tabela())
        Form.exec()
    ...
    usuario_selecionado = False
    def alterar_usuario(self):
        try:
            linha = self.tabela.selectedItems()
            # if(linha[0].text() != '1'):
            self.usuario_selecionado = linha[0].text()
            self.Janela.close()
        except: pass
    ...
    def mudar_filtro(self):
        ...
        if(self.filtro.currentIndex() == 0):
            ...
            self.busca.textChanged.connect(self.eventos_busca)
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (NOME DO USUÁRIO)")
        elif(self.filtro.currentData() == 2):
            # self.busca.setInputMask("999999999999999999999999999999999999999999999999")
            self.busca.disconnect()
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CÓDIGO DO USUÁRIO)")
    ...
    def carregar_tabela(self, opcao=0):
        ...
        opcao = SQL.pegar_usuario(lim, self.opcao_filtro) if(opcao == 0) else SQL.pegar_usuario_codigo(busca, lim) if(opcao == 2) else SQL.pegar_usuario_nome(busca, lim, self.opcao_filtro)
