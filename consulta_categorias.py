...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_categorias.ui'
#
...
class Consulta_categorias(object):
    def setupUi(self, Form):
        Form.setObjectName("Form")
        Form.resize(758, 424)
        Form.setMinimumSize(QtCore.QSize(758, 424))
        Form.setMaximumSize(QtCore.QSize(758, 424))
        icon = QtGui.QIcon()
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA CATEGORIA - ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA CATEGORIA DE EQUIPAMENTOS"))
        self.busca.setPlaceholderText(_translate("Form", "DIGITE SUA BUSCA AQUI"))
        self.filtro.setItemText(0, _translate("Form", "ESCOLHA OPCAO "))
        self.filtro.setItemText(1, _translate("Form", "CATEGORIA"))
        ...
        item.setText(_translate("Form", "SITUACAO"))
    ...
    def carregar_janela(self, Form):
        ...
        self.tabela.doubleClicked.connect(self.alterar_categoria)
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "CATEGORIAS", self.pegar_dados_tabela())
        Form.exec()
    ...
    categoria_selecionado = False
    def alterar_categoria(self):
        try:
            linha = self.tabela.selectedItems()
            self.categoria_selecionado = linha[0].text()
            self.Janela.close()
        except: pass
    ...
    def mudar_filtro(self):
        ...
        if(self.filtro.currentIndex() == 0):
            self.carregar_tabela()
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (NOME DO CATEGORIA)")
            ...
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CÓDIGO DO CATEGORIA)")
            # self.busca.setInputMask("9999999999999999999999999999999999")

        elif(self.filtro.currentIndex() == 3):
            self.busca.setInputMask("")
            self.busca.textChanged.connect(self.eventos_busca)
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CPF DO CATEGORIA)")


    def eventos_busca(self):
        self.busca.setText(self.busca.text().upper())
    ...
    def carregar_tabela(self, opcao=0):
        ...
        opcao = SQL.pegar_categorias(lim, self.opcao_filtro) if(opcao == 0) else SQL.pegar_categorias_servicos_nome(busca, lim, self.opcao_filtro) \
        if(opcao == 1) else SQL.pegar_categorias_servicos_codigo(busca, lim)

        self.tabela.setRowCount(0)
        for col, dados in enumerate(opcao):
            self.tabela.insertRow(self.tabela.rowCount())
            for lin, usu in enumerate(dados):
                usu = 'ATIVADO' if(lin == 3 and usu == 1) else "DESATIVADO" if(lin == 3 and usu == 0) else usu
                usu = 'SERVIÇOS' if(lin == 2 and usu == 1) else 'PRODUTOS' if(lin == 2 and usu == 0) else usu
