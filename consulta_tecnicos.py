...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_tecnico.ui'
#
...
class Consulta_tecnicos(object):
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA TECNICOS - ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA TECNICOS"))
        ...
        self.filtro.setItemText(3, _translate("Form", "CPF"))
    ...
    def carregar_janela(self, Form):
        ...
        self.tabela.doubleClicked.connect(self.alterar_tecnico)
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "TECNICOS", self.pegar_dados_tabela())
        Form.exec()
    ...
    tecnicos_selecionado = False
    def alterar_tecnico(self):
        try:
            linha = self.tabela.selectedItems()
            self.tecnicos_selecionado = linha[0].text()
            self.Janela.close()
        except: pass
    ...
    def mudar_filtro(self):
        ...
        if(self.filtro.currentIndex() == 0):
            self.carregar_tabela()
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (NOME DO TECNICOS)")
            ...
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CÓDIGO DO TECNICOS)")

        elif(self.filtro.currentIndex() == 3):
            self.busca.setInputMask("")
            self.busca.textChanged.connect(self.eventos_busca)
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CPF DO TECNICOS)")
    ...
    def carregar_tabela(self, opcao=0):
        ...
        opcao = SQL.pegar_tecnicos(lim, self.opcao_filtro) if(opcao == 0) else SQL.pegar_tecnicos_nome(busca, lim, self.opcao_filtro) if(opcao == 1) else SQL.pegar_tecnicos_codigo(busca, lim)\
        if(opcao == 2) else SQL.pegar_tecnicos_documento(busca, lim, self.opcao_filtro)
