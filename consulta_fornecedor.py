...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_fornecedor.ui'
#
...
class Consulta_fornecedor(object):
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA FORNECEDOR - ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA FORNECEDOR"))
        ...
        item.setText(_translate("Form", "CNPJ/CPF"))
    ...
    def carregar_janela(self, Form):
        ...
        self.tabela.doubleClicked.connect(self.alterar_fornecedor)
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "FORNECEDOR", self.pegar_dados_tabela())
        Form.exec()
    ...
    fornecedores_selecionado = False
    def alterar_fornecedor(self):
        try:
            linha = self.tabela.selectedItems()
            self.fornecedores_selecionado = linha[0].text()
            self.Janela.close()
        except: pass
    ...
    def carregar_tabela(self, opcao=0):
        ...
        opcao = SQL.pegar_fornecedores(lim, self.opcao_filtro) if(opcao == 0) else \
        SQL.pegar_fornecedores_nome(busca, lim, self.opcao_filtro) if(opcao == 1) else SQL.pegar_fornecedores_codigo(busca, lim)\
        if(opcao == 2) else SQL.pegar_cliente_documento(busca, lim, self.opcao_filtro)
