...
# Form implementation generated from reading ui file 'C:\Users\SUPORTE05\Documents\Meus Programas\apps\ASSISTENCIA TECNICA\UI\consulta_cliente.ui'
#
...
class Consulta_cliente(object):
    def setupUi(self, Form):
        ...
        Form.resize(889, 676)
        Form.setMinimumSize(QtCore.QSize(889, 469))
        Form.setMaximumSize(QtCore.QSize(889, 676))
        icon = QtGui.QIcon()
...
"\n"
"#Form{\n"
"image: url(:/img/consulta_cliente.jpg);\n"
"}\n"
"\n"
"QWidget{    \n"
...
"}")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CONSULTA CLIENTE- ASSISTENCIA TECNICA"))
        self.label.setText(_translate("Form", "CONSULTA CLIENTE"))
        self.busca.setPlaceholderText(_translate("Form", "DIGITE SUA BUSCA AQUI"))
        self.filtro.setItemText(0, _translate("Form", "SELECIONE CLIENTE"))
        self.filtro.setItemText(1, _translate("Form", "NOME"))
        self.filtro.setItemText(2, _translate("Form", "CÓDIGO"))
        self.filtro.setItemText(3, _translate("Form", "CPF/CNPJ"))
    ...
    def abrir_opcaoes_filtro(self):
        ...
        ui = Opcoes_filtro()
        ui.setupUi(Form, self.opcao_filtro, "CLIENTES", self.pegar_dados_tabela())
        Form.exec()
    ...
    clientes_selecionado = False
    def alterar_categoria(self):
        try:
            linha = self.tabela.selectedItems()
            self.clientes_selecionado = linha[0].text()
            self.Janela.close()
        except: pass
    ...
    def mudar_filtro(self):
        ...
        if(self.filtro.currentIndex() == 0):
            ...
            self.busca.textChanged.connect(self.eventos_busca)
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (NOME DO CLIENTES)")

        elif(self.filtro.currentIndex() == 2):
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CÓDIGO DO CLIENTES)")
            # self.busca.setInputMask("999999999999999999999999999999999999999999999999")

        elif(self.filtro.currentIndex() == 3):
            self.busca.setInputMask("")
            self.busca.textChanged.connect(self.eventos_busca)
            self.busca.setPlaceholderText("DIGITE SUA BUSCA AQUI (CNPJ/CPF DO CLIENTES)")
    ...
    def carregar_tabela(self, opcao=0):
        ...
        opcao = SQL.pegar_clientes(lim, self.opcao_filtro) if(opcao == 0) else SQL.pegar_cliente_nome(busca, lim, self.opcao_filtro) \
        if(opcao == 1) else SQL.pegar_cliente_codigo(busca, lim)  if(opcao == 2) else SQL.pegar_cliente_documento(busca, lim, self.opcao_filtro)

        self.tabela.setRowCount(0)
        for col, dados in enumerate(opcao):
            self.tabela.insertRow(self.tabela.rowCount())
            for lin, usu in enumerate(dados):
                usu = 'ATIVADO' if(lin == 6 and usu == 1) else 'DESATIVADO' if(lin == 6 and usu == 0) else usu
